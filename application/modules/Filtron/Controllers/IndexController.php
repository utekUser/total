 ...
 *
 */
class Filtron_controllers_IndexController extends Core_Controller_Action_User {
    /**
     * Инициализация
     *
     */
    ...
    public function indexAction() {
        ...
        $section = new Filtron_Models_Section();
        $article = new Filtron_Models_Article();
		$sectionName = null;
        // если выбран раздел
        if ($this->url) {
		...
		if ($request->getQuery('codeOpt')) {
			...
			if (sizeof($codeOpt)) {
        	    $this->view->paginator = $article->getFilterByCodeOpt($codeFilter);
        	    $this->view->codeOpt = $codeOpt;
        	} else {
        	    $this->view->paginator = $article->getArticle($sectionName['fk_id']);
        	}
    ...
    public function viewAction() {
		...
		$ttt = new Filtron_Models_Article();

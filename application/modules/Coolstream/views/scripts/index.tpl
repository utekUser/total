...
<div>
    <p><b>Подбор необходимого антифриза можно осуществить на сайте <a href="http://www.cool-stream.ru/catalog/selection/" target="_blank">производителя</a></b></p>
</div>
<div class="search-blockN">
    ...
    <div class="grey-corner-mN">
        <div class="search-opt">
            <form action="/coolstream/" method="get">
...
<div class="search-results">
    <div class="item-info">
        ...
        <input type="hidden" id="catalog-type" value="coolstream" />
        <div class="oiltableN">
            <style>
                th, td {
                    padding: 8px;
                ...
                <tbody>
                    ...
                    <?php if ($price != 'noshow') { ?>
						<tr>
							...
							<td class="item-price">
								<p class="oilText"><?php echo number_format($plug['warehouse_tver'] + $plug['warehouse_snab'] + $plug['warehouse_snabfilt'], 0, ' ', ' '); ?>&nbsp;шт.</p>
							</td>
							<td class="filterTitle"><?php echo $plug['articlesaler']; ?></td>
							<td class="item-price">
								<p class="oilText"><?php echo $price; ?>&nbsp;руб.</p>
							</td>
                                                        <?php if ($plug['warehouse_tver'] + $plug['warehouse_snab'] + $plug['warehouse_snabfilt'] == 0) : ?>
                                                            <td colspan="2">
							...
							<?php else : ?>
                                                            <td class="item-amount">
                                                                    ...
                                                                    <?php if (isset($_SESSION['basket']['coolstream'][$plug['id']])) { ?>
                                                                    <?php $value = $_SESSION['basket']['coolstream'][$plug['id']]; ?>
                                                                    <div class="input-field" style="width: 42px;"><div><div>
                                                                        <input type="text" name="price" id="<?php echo $plug['id']; ?>" value="<?php echo $value; ?>" onchange="changeAmount(this, 'coolstream', <?php echo $plug['id']; ?> );" />
                                                                        <input type="hidden" name="sumCount" id="sumCount<?php echo $plug['id']; ?>" value="<?php echo $plug['warehouse_tver'] + $plug['warehouse_snab'] + $plug['warehouse_snabfilt']; ?>" />
                                                                    </div></div></div>
                                                                    <?php } else { ?>
                                                                    <div class="input-field" style="width: 42px;"><div><div>
                                                            ...
                                                            <td class="item-action">
                                                                    <?php if ($this->auth_id) { ?>
                                                                    <?php if (isset($_SESSION['basket']['coolstream'][$plug['id']])) { ?>
                                                                    <a class="delete-item" href="javascript:{};" onclick="deleteItem(this, 'coolstream', <?php echo $plug['id']; ?> );">Удалить</a>
                                                                    <?php } else { ?>
                                                                    <a class="add-to-bask" href="javascript:{};" onclick="addItem(this, 'coolstream', <?php echo $plug['id']; ?> );">В корзину</a>
                                                                    <?php } ?>
                                                                    <?php } else { ?>
